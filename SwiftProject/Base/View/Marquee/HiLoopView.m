...
@interface HiLoopView () <HiLoopViewTouchResponder>

@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, assign) NSInteger visibleItemCount;
@property (nonatomic, strong) NSMutableArray<HiLoopListView*> *items;
@property (nonatomic, assign) int firstItemIndex;
@property (nonatomic, assign) int dataIndex;
@property (nonatomic, strong) NSTimer *scrollTimer;
@property (nonatomic, strong) HiLoopViewTouchReceiver *touchReceiver;
...
//                                                      userInfo:nil
//                                                       repeats:NO];
    self.scrollTimer = [NSTimer timerWithTimeInterval:timeInterval target:self selector:@selector(scrollTimerDidFire:) userInfo:nil repeats:NO];
...
- (void)resetAll {
    ...
    if (_direction == HiLoopViewDirectionLeftward) {
        self.visibleItemCount = 1;
    } else {
        if ([_delegate respondsToSelector:@selector(numberOfVisibleItemsForLoopView:)]) {
            self.visibleItemCount = [_delegate numberOfVisibleItemsForLoopView:self];
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        HiLoopListView *itemView = [[HiLoopListView alloc] init];
    ...
    if (_direction == HiLoopViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            int index = (i + self.firstItemIndex) % _items.count;
        ...
        if (_useDynamicHeight) {
            ...
            if ([_delegate respondsToSelector:@selector(numberOfDataForLoopView:)]) {
                dataCount = [_delegate numberOfDataForLoopView:self];
            }
...
- (int)itemIndexWithOffsetFromTop:(int)offsetFromTop {
    return (self.firstItemIndex + offsetFromTop) % (_visibleItemCount + 2);
}
...
- (CGFloat)itemWidthAtIndex:(NSInteger)index {
    ...
    if (index >= 0) {
        if ([_delegate respondsToSelector:@selector(itemViewWidthAtIndex:forLoopView:)]) {
            itemWidth = [_delegate itemViewWidthAtIndex:index forLoopView:self];
        }
    }
    return itemWidth;
}
...
- (CGFloat)itemHeightAtIndex:(NSInteger)index {
    ...
    if (index >= 0) {
        if ([_delegate respondsToSelector:@selector(itemViewHeightAtIndex:forLoopView:)]) {
            itemHeight = [_delegate itemViewHeightAtIndex:index forLoopView:self];
        }
    }
    return itemHeight;
}

- (void)createItemView:(HiLoopListView*)itemView {
    if (!itemView.didFinishCreate) {
        if ([_delegate respondsToSelector:@selector(createItemView:forLoopView:)]) {
            [_delegate createItemView:itemView forLoopView:self];
            itemView.didFinishCreate = YES;
        }
    }
}

- (void)updateItemView:(HiLoopListView*)itemView atIndex:(NSInteger)index {
    if (index < 0) {
        [itemView clear];
    }
    ...
    if (index >= 0) {
        if ([_delegate respondsToSelector:@selector(updateItemView:atIndex:forLoopView:)]) {
            [_delegate updateItemView:itemView atIndex:index forLoopView:self];
        }
    }
}
...
- (void)scrollTimerDidFire:(NSTimer *)timer {
    ...
    if (_stopWhenLessData) {
        ...
        if (_direction == HiLoopViewDirectionLeftward) {
            if (dataCount <= 1) {
                CGFloat itemWidth = MAX(self.items[1].width + self.itemSpacing, CGRectGetWidth(self.frame));
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        if (self.direction == HiLoopViewDirectionLeftward) {
            [self moveToNextDataIndex];
            ...
            for (int i = 0; i < self.items.count; i++) {
                int index = (i + self.firstItemIndex) % self.items.count;
                
                CGFloat itemWidth = MAX(self.items[index].width + self.itemSpacing, CGRectGetWidth(self.frame));
            ...
            self.items[self.firstItemIndex].width = [self itemWidthAtIndex:self.items[self.firstItemIndex].tag];
            CGFloat nextItemWidth = MAX(self.items[self.firstItemIndex].width + self.itemSpacing, CGRectGetWidth(self.frame));
            [self.items[self.firstItemIndex] setFrame:CGRectMake(lastItemWidth, 0.0f, nextItemWidth, itemHeight)];
            ...
            [self updateItemView:self.items[self.firstItemIndex] atIndex:self.items[self.firstItemIndex].tag];
            
            __weak __typeof(self) weakSelf = self;
            [UIView animateWithDuration:(currentItemWidth / self.scrollSpeed) delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
                CGFloat lastMaxX = 0.0f;
                for (int i = 0; i < self.items.count; i++) {
                    int index = (i + self.firstItemIndex) % self.items.count;
                    ...
                    if (i == 0) {
                        ...
                        [self.items[index] setFrame:CGRectMake(-itemWidth, 0.0f, itemWidth, itemHeight)];
                        lastMaxX = 0.0f;
                    } else {
                        [self.items[index] setFrame:CGRectMake(lastMaxX, 0.0f, itemWidth, itemHeight)];
                        lastMaxX = lastMaxX + itemWidth;
                    }
                }
            ...
            CGFloat itemWidth = CGRectGetWidth(self.frame);
            CGFloat itemHeight = CGRectGetHeight(self.frame) / self.visibleItemCount;
            
            // move the top item to bottom without animation
            self.items[self.firstItemIndex].tag = self.dataIndex;
            if (self.useDynamicHeight) {
                CGFloat firstItemWidth = self.items[self.firstItemIndex].height;
                self.items[self.firstItemIndex].height = [self itemHeightAtIndex:self.items[self.firstItemIndex].tag];
                [self.items[self.firstItemIndex] setFrame:CGRectMake(0.0f, CGRectGetMaxY(self.bounds), itemWidth, self.items[self.firstItemIndex].height)];
                ...
                [self.items[self.firstItemIndex] setFrame:CGRectMake(0.0f, CGRectGetMaxY(self.bounds), itemWidth, itemHeight)];
            }
            [self updateItemView:self.items[self.firstItemIndex] atIndex:self.items[self.firstItemIndex].tag];
            
            if (self.useDynamicHeight) {
                int lastItemIndex = (int)(self.items.count - 1 + self.firstItemIndex) % self.items.count;
                CGFloat lastItemHeight = self.items[lastItemIndex].height;
                __weak __typeof(self) weakSelf = self;
                [UIView animateWithDuration:(lastItemHeight / self.scrollSpeed) delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
                    for (int i = 0; i < self.items.count; i++) {
                        int index = (i + self.firstItemIndex) % self.items.count;
                        if (i == 0) {
                            continue;
                        } else if (i == 1) {
                            [self.items[index] setFrame:CGRectMake(CGRectGetMinX(self.items[index].frame),
                                                               CGRectGetMinY(self.items[index].frame) - lastItemHeight,
                                                               itemWidth,
                                                               self.items[index].height)];
                            self.items[index].alpha = 0.0f;
                        } else {
                            [self.items[index] setFrame:CGRectMake(CGRectGetMinX(self.items[index].frame),
                                                               CGRectGetMinY(self.items[index].frame) - lastItemHeight,
                                                               itemWidth,
                                                               self.items[index].height)];
                            self.items[index].alpha = 1.0f;
                        }
                    }
                } completion:^(BOOL finished) {
                    __strong __typeof(self) self = weakSelf;
                ...
                __weak __typeof(self) weakSelf = self;
                [UIView animateWithDuration:self.timeDurationPerScroll delay:0.0 options:animationOptions animations:^{
                    for (int i = 0; i < self.items.count; i++) {
                        int index = (i + self.firstItemIndex) % self.items.count;
                        if (i == 0) {
                            continue;
                        } else if (i == 1) {
                            [self.items[index] setFrame:CGRectMake(0.0f, -itemHeight, itemWidth, itemHeight)];
                        } else {
                            [self.items[index] setFrame:CGRectMake(0.0f, itemHeight * (i - 2), itemWidth, itemHeight)];
                        }
                    }
                } completion:^(BOOL finished) {
                    __strong __typeof(self) self = weakSelf;
...
- (void)resetTouchReceiver {
    if (_touchEnabled) {
        if (!_touchReceiver) {
            self.touchReceiver = [[HiLoopViewTouchReceiver alloc] init];
...
- (void)touchesEndedAtPoint:(CGPoint)point {
    for (HiLoopListView *itemView in self.items) {
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                if ([self.delegate respondsToSelector:@selector(didTouchItemViewAtIndex:forLoopView:)]) {
                    [self.delegate didTouchItemViewAtIndex:itemView.tag forLoopView:self];
                }
            }
            break;
        }
...
@implementation HiLoopViewTouchReceiver
