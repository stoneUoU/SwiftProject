...
// MARK: - HiLoopViewItem
open class HiLoopViewItem: UIView {
...
public protocol HiLoopViewDataSource: AnyObject {
...
public final class HiLoopView: UIView {
    
    // MARK: - Properties
    public weak var dataSourse: HiLoopViewDataSource?
    ...
    public var space: CGFloat = 24
    private var offset: CGFloat = 48
    
    private var currentIndex = 0
    private var itemCount = 0
    private var oldFrame: CGRect?
    
    private var visibleItems: [HiLoopViewItem] = []
    private var reuseCache: [String: HiLoopQueue<HiLoopViewItem>] = [:]
    ...
    public func dequeueReusableItem(withIdentifier identifier: String) -> HiLoopViewItem {
        if let item = reuseCache[identifier]?.dequeue() {
            return item
        }
        
        guard let itemClass = classCache[identifier] as? HiLoopViewItem.Type else {
    ...
    private func reuseItem(_ item: HiLoopViewItem) {
        item.removeFromSuperview()
        if reuseCache[item.reuseIdentifier] == nil {
            reuseCache[item.reuseIdentifier] = HiLoopQueue<HiLoopViewItem>()
        }
